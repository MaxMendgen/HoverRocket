namespace OpenCvSharp.Demo
{
	...
	/// </summary>
	public class EDITEDTrackingScript
		: WebCamera
		, IBeginDragHandler
		, IDragHandler
		, IEndDragHandler
	{
		...
		[SerializeField] private TextMeshProUGUI RealBallPositionDisplay;

		private Transform ball;

		[HideInInspector] public float ballPosition;
		[SerializeField] private int camWidth = 540;
		[SerializeField] private float MaxBallMovementMultiplyer = 4;
		[SerializeField] private float BallMidpointOffset;
		...
		protected override bool ProcessTexture(WebCamTexture input, ref Texture2D output)
		{
			...
			if (!isDragging)
			{
				...
				if (0 != obj.Width && 0 != obj.Height)
				{
					...
					print("X");
					print(obj.X);
					ballPosition = (System.Convert.ToSingle(obj.X) - camWidth / 2) / camWidth * MaxBallMovementMultiplyer + BallMidpointOffset;
					RealBallPositionDisplay.text = $"Real Ball Position: {ballPosition:F2}";

					ball.position = new Vector3(ballPosition, ball.transform.position.y, ball.transform.position.z);
